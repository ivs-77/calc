...
1. `port=1500` - порт, который будет слушать приложение.
...
1. `result_format=%14.3f` - формат, в котором приложение выдает результат вычислений. Результат форматируется функцией sprintf, соответственно данный параметр должен определять формат числа с плавающей точкой для данной фунции.
# Настройка БД
